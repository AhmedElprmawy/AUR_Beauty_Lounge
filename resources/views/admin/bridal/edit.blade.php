...
@section('title', 'تعديل قسم العرائس')

@section('page-title', '✏️ تعديل قسم العرائس')
...
<div class="form-container">
    <form action="{{ route('admin.bridal.update', $bridal->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="text" name="title_ar" value="{{ old('title_ar', $bridal->title_ar) }}" placeholder="مثال: تجهيز العرائس">
                <div class="helper-text">يظهر في العنوان الكبير</div>
            </div>
            <div class="form-group">
                <label>العنوان الرئيسي (إنجليزي)</label>
                <input type="text" name="title_en" value="{{ old('title_en', $bridal->title_en) }}" placeholder="مثال: Bridal Preparation">
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <textarea name="description_ar" rows="4" placeholder="وصف القسم...">{{ old('description_ar', $bridal->description_ar) }}</textarea>
        </div>

        <div class="form-group">
            <label>الوصف (إنجليزي)</label>
            <textarea name="description_en" rows="4" placeholder="Description...">{{ old('description_en', $bridal->description_en) }}</textarea>
        </div>
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="text" name="image" value="{{ old('image', $bridal->image) }}" placeholder="https://example.com/image.jpg">
                @if($bridal->image)
            ...
            <div class="form-group">
                ...
                <input type="text" name="small_image" value="{{ old('small_image', $bridal->small_image) }}" placeholder="https://example.com/small.jpg">
                @if($bridal->small_image)
                    <div class="image-preview">
                        <img src="{{ $bridal->small_image }}" alt="الصورة الصغيرة">
                    </div>
                @endif
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="text" name="stats_number" value="{{ old('stats_number', $bridal->stats_number) }}" placeholder="مثال: 1500+">
                <div class="helper-text">الرقم الذي يظهر في البطاقة العائمة</div>
            </div>
            <div class="form-group">
                <label>تسمية الإحصائية (عربي)</label>
                <input type="text" name="stats_label_ar" value="{{ old('stats_label_ar', $bridal->stats_label_ar) }}" placeholder="مثال: عروس جميلة">
            </div>
        </div>

        <div class="form-group">
            <label>تسمية الإحصائية (إنجليزي)</label>
            <input type="text" name="stats_label_en" value="{{ old('stats_label_en', $bridal->stats_label_en) }}" placeholder="مثال: Beautiful Brides">
        </div>
        ...
        <div class="checkbox-group">
            <input type="checkbox" name="is_active" id="is_active" {{ $bridal->is_active ? 'checked' : '' }}>
