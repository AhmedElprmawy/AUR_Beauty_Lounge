...
@section('page-title', '✏️ تعديل موظف')
    ...
    <form action="{{ route('admin.staff.update', $staff) }}" method="POST">
        ...
        <div class="row-2">
            <div class="form-group">
                ...
                <input type="text" name="name_ar" value="{{ old('name_ar', $staff->name_ar) }}" required>
            </div>
            <div class="form-group">
                <label>الاسم (إنجليزي)</label>
                <input type="text" name="name_en" value="{{ old('name_en', $staff->name_en) }}">
            </div>
        </div>
        ...
        <div class="row-2">
            <div class="form-group">
                ...
                <input type="text" name="role_ar" value="{{ old('role_ar', $staff->role_ar) }}" required>
            </div>
            <div class="form-group">
                <label>الدور (إنجليزي)</label>
                <input type="text" name="role_en" value="{{ old('role_en', $staff->role_en) }}">
            </div>
        </div>
        ...
        <div class="row-2">
            <div class="form-group">
                ...
                <input type="number" name="experience_years" min="0" value="{{ old('experience_years', $staff->experience_years) }}">
            </div>
            <div class="form-group" style="display:flex; align-items:center; gap:10px; padding-top: 32px;">
                <input type="checkbox" name="is_active" id="is_active" {{ old('is_active', $staff->is_active) ? 'checked' : '' }}>
        ...
        <div class="form-group">
            ...
            <textarea name="bio_ar" rows="4">{{ old('bio_ar', $staff->bio_ar) }}</textarea>
        </div>

        <div class="form-group">
            <label>نبذة (إنجليزي)</label>
            <textarea name="bio_en" rows="4">{{ old('bio_en', $staff->bio_en) }}</textarea>
        </div>

        <div class="row-2">
            <div class="form-group">
                <label>رابط الصورة</label>
                <input type="text" name="image" value="{{ old('image', $staff->image) }}">
            </div>
            <div class="form-group">
                <label>Instagram</label>
                <input type="text" name="instagram" value="{{ old('instagram', $staff->instagram) }}">
            </div>
        </div>

        <div class="form-group">
            <label>Twitter</label>
            <input type="text" name="twitter" value="{{ old('twitter', $staff->twitter) }}">
        </div>
