...
@section('title', 'تعديل باقة عرائس')

@section('page-title', '✏️ تعديل باقة العرائس')
...
<div class="form-container">
    ...
    <div class="info-box">
        <div>
            <span class="label">🆔 رقم الباقة:</span>
            <span class="value">#{{ $package->id }}</span>
        </div>
        <div>
            <span class="label">📅 تاريخ الإنشاء:</span>
            <span class="value">{{ $package->created_at->format('Y-m-d H:i') }}</span>
        </div>
        <div>
        ...
        <div>
            <span class="label">⭐ مميزة:</span>
            <span class="status-badge {{ $package->is_popular ? 'status-active' : 'status-inactive' }}">
                {{ $package->is_popular ? '✅ نعم' : '❌ لا' }}
            </span>
        </div>
        <div>
            <span class="label">🏷️ التصنيف:</span>
            <span class="tier-badge tier-{{ $package->tier }}">
    ...
    <form action="{{ route('admin.packages.update', $package) }}" method="POST">
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="text" name="name_ar" value="{{ old('name_ar', $package->name_ar) }}" required placeholder="مثال: عروس ذهبية">
                @error('name_ar')
            ...
            <div class="form-group">
                ...
                <input type="text" name="name_en" value="{{ old('name_en', $package->name_en) }}" placeholder="مثال: Gold Bride">
                @error('name_en')
                    <div style="color: var(--danger-color); font-size: 13px; margin-top: 4px;">{{ $message }}</div>
                @enderror
            </div>
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <select name="tier" required>
                    <option value="silver" {{ old('tier', $package->tier) == 'silver' ? 'selected' : '' }}>🥈 فضية (Silver)</option>
                    <option value="gold" {{ old('tier', $package->tier) == 'gold' ? 'selected' : '' }}>🥇 ذهبية (Gold)</option>
                    <option value="platinum" {{ old('tier', $package->tier) == 'platinum' ? 'selected' : '' }}>💎 بلاتينية (Platinum)</option>
                </select>
                @error('tier')
                    <div style="color: var(--danger-color); font-size: 13px; margin-top: 4px;">{{ $message }}</div>
                @enderror
            ...
            <div class="form-group">
                <label>السعر *</label>
                <input type="number" name="price" min="0" step="0.01" value="{{ old('price', $package->price) }}" required placeholder="مثال: 5000">
                @error('price')
                    <div style="color: var(--danger-color); font-size: 13px; margin-top: 4px;">{{ $message }}</div>
                @enderror
            </div>
        ...
        <div class="form-row">
            <div class="form-group">
                <label>العملة *</label>
                <input type="text" name="currency" value="{{ old('currency', $package->currency) }}" required placeholder="مثال: SAR, ج.م">
                @error('currency')
                    <div style="color: var(--danger-color); font-size: 13px; margin-top: 4px;">{{ $message }}</div>
                @enderror
            </div>
            <div style="display: flex; gap: 20px; align-items: center; padding-top: 20px;">
                <div class="checkbox-group" style="padding-top: 0;">
                    <input type="checkbox" name="is_popular" id="is_popular" value="1" {{ old('is_popular', $package->is_popular) ? 'checked' : '' }}>
        ...
        <div class="form-group">
            ...
            <textarea name="description_ar" rows="3" placeholder="وصف الباقة بالعربية...">{{ old('description_ar', $package->description_ar) }}</textarea>
        ...
        <div class="form-group">
            ...
            <textarea name="description_en" rows="3" placeholder="Description in English...">{{ old('description_en', $package->description_en) }}</textarea>
        ...
        <div class="form-group">
            ...
            <div class="features-container">
                <div id="featuresArList">
                    @if($package->features_ar && count($package->features_ar) > 0)
        ...
        <div class="form-group">
            ...
            <div class="features-container">
                <div id="featuresEnList">
                    @if($package->features_en && count($package->features_en) > 0)
                        @foreach($package->features_en as $feature)
                            <div class="feature-item">
        ...
        <div class="form-group">
            <label>📊 الترتيب</label>
            <input type="number" name="order" value="{{ old('order', $package->order) }}" placeholder="0">
        ...
        <div style="display: flex; gap: 15px; margin-top: 30px; flex-wrap: wrap;">
            ...
            <button type="button" class="btn-danger" onclick="confirmDelete({{ $package->id }})">🗑️ حذف الباقة</button>
        </div>
...
<script>
    ...
    function confirmDelete(id) {
        if (confirm('⚠️ هل أنت متأكد من حذف هذه الباقة؟ لا يمكن التراجع عن هذا الإجراء!')) {
